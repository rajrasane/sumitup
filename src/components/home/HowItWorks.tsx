...
function StepItem({icon,label,description} : Step ){
    return <div className="relative p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 hover:border-rose-500/50 transition-colors group w-full">
            ...
            <div className="py-12 lg:py-24 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 ">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto relative">
                    {steps.map((step, index) => (
                        <div key={index} className="relative flex item-stretch">
                            <StepItem {...step}/>
                            <div className="hidden md:block absolute top-1/2 -right-4 transform -translate-y-1/2 z-10">
                                {
                                    index < steps.length - 1 ? 
                                    <MoveRight size={32} strokeWidth={1} className="text-rose-400"/> : null
                                }
                            </div>
                        </div>
                    ))}
